...
namespace GUARD
{
    namespace pe_constructor
    {
        ...
        pe_image virtual_to_raw_image( pe_image& virtual_image )
        {
            ...
            image_x86_t* img = virtual_image.get_image();
            nt_headers_x86_t* nt = img->get_nt_headers();
            ...
            for ( int i = 0; i < nt->file_header.num_sections; i++ )
            {
                ...
                //
                uint32_t section_end = section->virtual_address + section->Misc.virtual_size;
                ...
                // Copy section bytes.
                //
                std::copy( virtual_raw_bytes.begin() + section->virtual_address, virtual_raw_bytes.begin() + section->virtual_address + section->Misc.virtual_size, raw_bytes.begin() + section->virtual_address );
            }
            
            // Construct the raw image.
            //
            pe_image raw_image = { raw_bytes };
            image_x86_t* raw_img = raw_image.get_image();
            nt_headers_x86_t* raw_nt = raw_img->get_nt_headers();
            ...
            for ( int i = 0; i < raw_nt->file_header.num_sections; i++ )
            {
                ...
                uint32_t required_alignment = section_alignment - ( section->Misc.virtual_size % section_alignment );

                section->ptr_raw_data = section->virtual_address;
                section->size_raw_data = section->Misc.virtual_size + required_alignment;
            }
        ...
        uint32_t get_sections_end( pe_image& virtual_image )
        {
            ...
            for ( int i = 0; i < nt->file_header.num_sections; i++ )
            {
                ...
                uint32_t required_alignment = nt->optional_header.section_alignment - ( section->Misc.virtual_size % nt->optional_header.section_alignment );

                uint32_t section_end = section->virtual_address + section->Misc.virtual_size + required_alignment;
        ...
        pe_image& add_section( pe_image& raw_image, const std::vector<uint8_t>& section, uint32_t va, const std::string& name, win::section_characteristics_t characteristics )
        {
            ...
            new_section.Misc.virtual_size = section.size();
            ...
            image_x86_t* img = raw_image.get_image();
            nt_headers_x86_t* nt = img->get_nt_headers();
            ...
            *nt->get_section( nt->file_header.num_sections ) = new_section;
            ...
            //
            if ( nt->optional_header.size_image < new_section.virtual_address + new_section.Misc.virtual_size )
                nt->optional_header.size_image = new_section.virtual_address + new_section.Misc.virtual_size + required_section_alignment;
