...
namespace GUARD
{
    ...
    struct import_call
    {
        ...
        uint32_t mov_reg_idx;
        uint32_t mov_reg_val;
        ...
        // Constructor.
        //
        import_call( uint64_t call_rva, const resolved_import* import, uint32_t mov_reg_idx, uint32_t mov_reg_val, int32_t stack_adjustment,
            bool padded, bool is_jmp, std::optional<instruction> prev_instruction = {} )
            : call_rva( call_rva ), import( import ), mov_reg_idx(mov_reg_idx), mov_reg_val(mov_reg_val), stack_adjustment( stack_adjustment ),
            padded( padded ), is_jmp( is_jmp ), prev_instruction( prev_instruction )
        {}
    };
}