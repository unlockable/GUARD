...
namespace GUARD
{
    namespace pe_constructor
    {
        ...
        //
        template <typename T>
        std::tuple<std::vector<uint8_t>, std::vector<uint32_t>, uint32_t> serialize_table( const std::vector<T>& table_entries, uint32_t offset_base = 0 )
        {
            // Result vectors.
            //
            ...
            for ( const T& entry : table_entries )
            {
                ...
                //
                //printf("result_offsets : %x\n", result_bytes.size() + offset_base);
                ...
                //printf("========= entry bytes.begin() : %x and entry bytes.end() : %x\n", entry_bytes.begin(), entry_bytes.end());
