...
namespace win
{
	...
	struct image_t
	{
		...
		inline operator dos_header_t&() {
			printf("dos_header_t&\n");
			return dos_header; 
		}
		inline dos_header_t& get_dos_headers() { return dos_header; }
		inline uint64_t get_dos_addr() { return (uint64_t)&dos_header; }

		inline nt_headers_t<x64>* get_nt_headers() { return dos_header.get_nt_headers<x64>(); }
		//inline nt_headers_t<x86>* get_nt_headers() { return dos_header.get_nt_headers<x86>(); }
		...
		inline data_directory_t* get_directory( directory_id id )
		{
			...
			auto dos_headers = get_dos_headers();
			printf("nt_hdrs : %d\n", nt_hdrs);
			printf("dos addr is %x\n", dos_headers.e_magic);
		...
		inline T* rva_to_ptr( uint32_t rva )
		{
			...
			for ( int i = 0; i < nt_hdrs->file_header.num_sections; i++ )
			{
				...
				if ( section->virtual_address <= rva && rva < ( section->virtual_address + section->Misc.virtual_size ) )
				{
