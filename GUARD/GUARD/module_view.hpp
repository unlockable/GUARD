...
namespace GUARD
{
    ...
    struct module_view
    {
        ...
        //
        module_view( uint32_t process_id, const std::string& module_name, const std::string& module_path, remote_ea_t module_base, size_t module_size )
            : process_id( process_id ), module_name( module_name ), module_path(module_path), module_base( module_base ), module_size( module_size )
        {
            fetch();
        }
        ...
        // Constructor.
        //
        module_view( uint32_t process_id, const std::string& module_name, const std::string& module_path,remote_ea_t module_base, size_t module_size, const pe_image& local_module )
            : process_id( process_id ), module_name( module_name ), module_path(module_path), module_base( module_base ), module_size( module_size ), local_module( local_module )
        {}
    };
}