...
namespace GUARD
{
    ...
    std::vector<instruction_t> disasm(const void* bytes, uint64_t address, size_t size, size_t count)
    {
        cs_insn* ins;
        count = cs_disasm(
            get_cs_handle(),
    ...
    std::optional<uint32_t> GUARD::generate_stub( uint32_t rva, remote_ea_t thunk )
    {
        ...
        if (ks_asm(ks, buffer, 0, &encode, &size, &count) != KS_ERR_OK) {
            ...
            jump = { encode, encode + size };

        }

        //std::vector<uint8_t> jump = assemble( format::str( "jmp [0x%p]", thunk ), target_module_view->module_base + stub_rva );
    ...
    bool GUARD::convert_local_call( const import_call& call, remote_ea_t thunk, remote_ea_t reg_val )
    {
        ...
        if ( call.stack_adjustment == bitoffset )
        {
            ...
            {                
                printf( "!! Stack adjustment failed1 for call @ RVA 0x%llx for thunk @ 0x%llx\r\n", call.call_rva, thunk );
                return false;
            }
        } 
        else if (call.stack_adjustment == -4)
        {
            ...
            {
                printf("!! Stack adjustment failed2 for call @ RVA 0x%llx for thunk @ 0x%llx\r\n", call.call_rva, thunk);
                return false;
            }
        }
        else if (call.stack_adjustment != 0) {
            printf("!! Stack adjustment failed3 for call @ RVA 0x%llx for thunk @ 0x%llx\r\n", call.call_rva, thunk);
            return false;
        }
        ...
        auto instructions = disasm( call_ea, call.call_rva, 0, 1 );

        // Ensure disassembly succeeded.
        //
        if ( instructions.empty() )
        {
            printf( "!! Disassembly failed for call @ RVA 0x%llx for thunk @ 0x%llx\r\n", call.call_rva, thunk );
            return false;
        }
    ...
    "R11W", "R12W", "R13W", "R14W", "R15W"
            ...
            if (thunk == target_module_view->module_base) {
                printf("!! Stack adjustment failed4 for call @ RVA 0x%llx for thunk @ 0x%llx\r\n", call.call_rva, thunk);
                return false;
            }
